...
export default function Notification() {
  const { show, title, body, icon } = useStore((store) => store.state);
  const { toggle: toggleNotification } = useStore((store) => store.actions);
  return (
    <>
              ...
              <div className="p-4">
                <div className="flex items-start">
                  ...
                  <div className="ml-3 w-0 flex-1 pt-0.5">
                    ...
                    {body && <div className="mt-1 text-sm text-gray-500">{body}</div>}
                  </div>
